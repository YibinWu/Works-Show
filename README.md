...
## 1. Wheel-INS 
In this study, a MEMS IMU was mounted on the wheel of the ground vehicle to take advantage of the rotation modulation as well as obtain the vehicle velocity to suppress the error drift of the Inetial Navigation System (INS). In addition, the single wheel-mounted IMU was extended to multi-IMU placed at different locations of the vehicle to enhance the localization performance. The absolute positioning error drift rate of the proposed system is less than **1%**. The core idea of this research is to use IMUs to perceive special dynamics (e.g., rotation of the wheel and gait of the legged robots) of parts of a vehicle to suppress the error drift of INS; then, fuse these diverse motion information (indicated by INS) to improve the  robustness and accuracy of the positioning results. Please click [here](https://github.com/i2Nav-WHU/Wheel-INS) for public code and data.
